      ...
      scope.step.getSentences = function() {
        $http.get('/project/psychological/v2/console/getSentences?type=' + scope.step.type + '&stage=' + scope.step.stage)
          .success(function(data) {
            ...
            } else if (parseInt(scope.part) === 1) {
              scope.step.sentences = data.sentences.splice(0, 80).sort(function() {
                return Math.random() > .5 ? -1 : 1;
              });
            } else if (parseInt(scope.part) === 2) {
              scope.step.sentences = data.sentences.splice(80, 80).sort(function() {
                return Math.random() > .5 ? -1 : 1;
              });
            } else if (parseInt(scope.part) === 3) {
              scope.step.sentences = data.sentences.splice(160, 80).sort(function() {
                return Math.random() > .5 ? -1 : 1;
              });
            } else if (parseInt(scope.part) === 4) {
              scope.step.sentences = data.sentences.splice(240, 80).sort(function() {
                return Math.random() > .5 ? -1 : 1;
              });
            } else if (parseInt(scope.part) === 5) {
              scope.step.sentences = data.sentences.splice(320, 80).sort(function() {
                return Math.random() > .5 ? -1 : 1;
              });
            } else if (parseInt(scope.part) === 6) {
              scope.step.sentences = data.sentences.splice(400, 80).sort(function() {
                return Math.random() > .5 ? -1 : 1;
              });
            } else if (parseInt(scope.part) === 7) {
              scope.step.sentences = data.sentences.splice(480, 80).sort(function() {
                return Math.random() > .5 ? -1 : 1;
              });
            } else if (parseInt(scope.part) === 8) {
              scope.step.sentences = data.sentences.splice(560, 80).sort(function() {
                return Math.random() > .5 ? -1 : 1;
              });
            } else {
